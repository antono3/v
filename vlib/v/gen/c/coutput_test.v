...
fn test_boehm_scope_pin_keeps_partially_declared_c_aggregate_reachable() {
	os.chdir(vroot) or {}
	test_source := os.join_path(vroot, 'vlib/v/gen/c/testdata/boehm_keepalive_c_anon_union.vv')
	cmd := '${os.quoted_path(vexe)} -old-compiler -prod -gc boehm_full_opt -o - ${os.quoted_path(test_source)}'
	compilation := os.execute(cmd)
	ensure_compilation_succeeded(compilation, cmd)
	assert !generated_c_uses_v3_codegen(compilation.output)
	// The generated helper prototype caused the anonymous-union GCC failure.
	assert !compilation.output.contains('TestAnonEvent* it, voidptr* out, int idx)')
	// The untyped pin must remain: the real C struct has a hidden pointer field.
	assert compilation.output.contains('GC_reachable_here(_t')
}
